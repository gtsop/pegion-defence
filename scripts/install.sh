...
RPI_NODE_FE_ROOT=$(realpath ./workspaces/rpi-node/frontend/public)
...
sudo mkdir -p /var/www/rpi-node-fe
sudo cp -R $RPI_NODE_FE_ROOT/* /var/www/rpi-node-fe

sudo mkdir -p /etc/nginx/nginx.d/
sudo cp ./workspaces/rpi-node/frontend/nginx/nginx.conf /etc/nginx/nginx.conf
